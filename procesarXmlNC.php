...
include_once("Conexion.php");
 ...
 function generarXml($id_cliente,$id_factura,$cufe,$nom_xml) {
    ...
    while (($row = oci_fetch_array($stid2, OCI_BOTH)) != false) {
 	    ...
 	    //Query sql data xml
	$sql = "SELECT CLIENTE.CLIENCODIG,CLIENTE.CLIENTIPID,CONCAT('SETT',FACTURA.FACTUCODIG) AS NUM_FACTURA,TO_CHAR(FACTURA.factufecge,'YYYY-MM-DD') AS Fecha,TO_CHAR(FACTURA.factufecge,'HH:MM:SS') AS Hora,CLIENTE.CLIENTIPO,
	        ...
	        FACTURA.factuvalne,FACTURA.factuvalor,ValidaIva(CLIENTE.cliencodig) AS IVA,FACTURA.factuvaliv ,FACTURA.factuvalfu,FACTURA.factuvalic,FACTURA.factuvalri, ValiMatMer (CLIENTE.CLIENTIPO,CLIENTE.cliencodig) AS MatMerc, ValiIdClient(CLIENTE.CLIENTIPO,CLIENTE.cliencodig) AS IdClientJu,
	        ValiIdClientTwo(CLIENTE.CLIENTIPO,CLIENTE.cliencodig) AS IdClientNa, ValIdIva(CLIENTE.cliencodig) AS Id_iva, ValiNomRut(CLIENTE.CLIENTIPO,CLIENTE.cliencodig) AS Nom_Rut,
            ...
            ValidaImpuestos(".($result[1][1]/100).",FACTUDETAL.facdevalto,FACTURA.factuvalri) as RTIVA ,ValidaImpuestos(".($result[0][1]/100).",FACTUDETAL.facdevalto,FACTURA.factuvaliv) as IVARES,TO_CHAR(sysdate,'YYYY-MM-DD') as fec_actual
    ...
    $suffix = null;
    $path = pathinfo('file:/archivosXML/'.$nom_xml.'.xml');
	...
	$objetoXML->startElement("NOTA CREDITO");
		  $objetoXML->startElement("ENC"); //elemento Encabezado
		        ...
		        $objetoXML->startElement("ENC_6"); //elemento Numero de NOTA CREDITO
		        $objetoXML->text('NDC_2');
		        $objetoXML->endElement(); //fin Numero de NOTA CERDITO
		        ...
		        $objetoXML->text('20');//valor por defecto NC=20
	      ...
	      $objetoXML->startElement("DRF"); //elemento Numeracion de factura
		        ...
		        $objetoXML->text('5000000');
		        $objetoXML->endElement(); //fin nombre del tributo (maximo)

	      $objetoXML->endElement(); //fin Numeracion de factura

	      $objetoXML->startElement("REF"); //elemento Numeracion de factura

	            $objetoXML->startElement("REF_1"); //elemento numero de autorizacion
		        $objetoXML->text('IV');
		        $objetoXML->endElement(); //fin numero de autorizacion

		        $objetoXML->startElement("REF_2"); //elemento Numero de factura de referencia
		        $objetoXML->text((string)$foo[0][2]);
		        $objetoXML->endElement(); //fin Numero de factura de referencia

		        $objetoXML->startElement("REF_3"); //elemento fecha
		        $objetoXML->text((string)$foo[0][34]);
		        $objetoXML->endElement(); //fin fec fin periodo de autorizacion

		        $objetoXML->startElement("REF_4"); //elemento Codigo Cufe
		        $objetoXML->text($cufe);
		        $objetoXML->endElement(); //Codigo Cufe

		        $objetoXML->startElement("REF_5"); //elemento rango numeracion (minimo)
		        $objetoXML->text('CUFE-SHA384');
		        $objetoXML->endElement(); //fin rango numeracion (minimo)
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
    DOCUMENTO DEL CLIENTE: <input type="text" name="id_client"><br>
    NUMERO DE FACTURA: <input type="text" name="id_fact"><br>
    CODIGO CUFE: <input type="text" name="cufe"><br>
...
<?php
if (isset($_POST['send'])){
    generarXml($_POST['id_client'],$_POST['id_fact'],$_POST['cufe'],"NC_".$_POST['id_fact']);
    echo "<BR> Se genero XML";
  }
